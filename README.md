# @shapeshift-labs/frontier-auth

Frontier-native auth contracts for applications that need authentication and authorization evidence without adopting a specific auth server, database, crypto package, or web framework.

`frontier-auth` models the pieces Frontier needs to inspect and enforce auth while leaving runtime auth handling to app-owned adapters:

- auth manifests for providers, session strategy, profile requirements, route/resource gates, capabilities, token contracts, runtime grants, and account-linking policy
- normalized session records and deterministic gate decisions
- token issue/verify plans that leave signing and verification to host-owned crypto adapters
- structural account-linking resolution for provider-first and email-fallback flows
- registry graphs, lint resources, redacted audit events, and evidence bundles for agents and CI
...
const auth = createAuthManifest({
  id: 'my-app.auth',
  providers: [
    { id: 'primary-oauth', kind: 'oauth', scopes: ['identify', 'email'], pkce: true }
  ],
  profile: {
    fields: ['username'],
...
const decision = evaluateAuthGate(auth, session, 'api.strict');
```

The package is dependency-light and works in Node, browser, edge, workers, CLIs, and game runtimes. Integrations with `frontier-framework`, `frontier-policy`, `frontier-tools`, `frontier-linter`, `frontier-test`, telemetry, CRDT/realtime transports, and app-owned identity stores are structural.

## Discord Adapter

`@shapeshift-labs/frontier-auth/discord` provides a Discord-specific Frontier adapter for apps that want provider defaults plus real authorization redirect and callback handling without moving secrets, token storage, user storage, or final session persistence into the package.

```ts
import { createDiscordAuthAdapter } from '@shapeshift-labs/frontier-auth/discord';

const discord = createDiscordAuthAdapter({
  ...
  identityStore: {
    findByProvider: (provider, providerAccountId) => users.findByProvider(provider, providerAccountId),
    findByEmail: (email) => users.findByEmail(email),
    linkProvider: (identity, providerIdentity) => users.linkProvider(identity, providerIdentity)
  },
  callbacks: {
    saveState: ({ flow }) => sessionState.save(flow.state, flow),
    loadState: ({ state }) => sessionState.load(state),
    deleteState: ({ state }) => sessionState.delete(state),
    createSession: ({ selectedIdentity, providerIdentity, user }) => ({
      subject: selectedIdentity?.id ?? providerIdentity.providerAccountId,
      provider: 'discord',
      providerAccountId: providerIdentity.providerAccountId,
      email: providerIdentity.email,
      username: selectedIdentity?.username ?? user.global_name ?? user.username
    })
  }
});

const start = await discord.start({
  returnTo: '/home',
  state: crypto.randomUUID()
});

// Send `start.redirect.status` and `start.redirect.headers` from your login route.

const callback = await discord.handleCallback({ url: request.url });
if (!callback.ok) {
  // Send `callback.redirect.status` and `callback.redirect.headers`.
}

// Persist callback.session and any token material your app intentionally stores.
```

The adapter verifies `state` by default, exchanges authorization codes with form-encoded requests, can refresh or revoke tokens, fetches the Discord user profile, resolves identities through the existing Frontier identity-store adapter, and returns a safe final redirect plan. Absolute final redirects are allowed only when they match `baseUrl` or `allowedRedirectOrigins`; relative redirects are resolved against `baseUrl` when present.
